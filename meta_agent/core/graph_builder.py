...
请根据上述角色定位和技能说明，处理用户的请求。"""
    ...
    async def _save_to_local_node(
        ...
        try:
            ...
            # 保存各平台内容（使用公共方法）
            saved_files = []
            platform_map = self._get_platform_file_map()
            ...
            for worker_name, result in worker_results.items():
                if result.get("success") and result.get("content"):
                    ...
                    if self._save_file_to_disk(filepath, result["content"]):
            ...
            if self._save_file_to_disk(summary_path, summary_content):
    ...
    def _save_file_to_disk(self, filepath: str, content: str) -> bool:
        """
        保存文件到磁盘的公共方法
    ...
    def _generate_summary_report(
        ...
        # 各平台内容预览
        platform_names = self._get_platform_names()

        for worker_name, result in worker_results.items():
            platform_name = platform_names.get(worker_name, worker_name)
            self._add_platform_section(report_lines, platform_name, result)

        # 文件列表
        self._add_file_list(report_lines, saved_files)

        return "\n".join(report_lines)

    def _get_platform_names(self) -> Dict[str, str]:
        """获取平台名称映射"""
        return {
            "@Douyin_Expert": "抖音",
    ...
    def _get_platform_file_map(self) -> Dict[str, str]:
        """获取平台文件名映射"""
        return {
    ...
    def _add_platform_section(self, report_lines: list, platform_name: str, result: Dict[str, Any]):
        """添加平台内容章节"""
    ...
    def _add_file_list(self, report_lines: list, saved_files: list):
        """添加文件列表章节"""
        report_lines.extend([
            "## 文件列表",
            ""
...
请以清晰、专业的方式组织你的回答，使用 Markdown 格式。"""
    ...
    def _should_continue_after_replan(
        ...
        if state.get("final_output"):
            logger.debug("✅ 所有任务已完成，进入 QA 质量检查节点")
            return "qa"
